  ...
  const mailOptions = {
    ...
    html: `<div
        ...
        <div
            ...
            <h3>Selamat Datang Di Rest-APIs Elxyz</h3>
            <p>
          ...
          <div align="center">
            <img src="https://camo.githubusercontent.com/fbfa09c159814f6e1a90cc6fe53b52053ec0a558abcceb8df8a1d9a2e18de494/68747470733a2f2f766965772e6d6f657a782e63632f696d616765732f323032312f30322f32352f37323137323934613863623939326433376563656562386635613031643130302e676966" width="200px" alt="kii">
           </div>
  ...
  async function(accessToken, refreshToken, profile, done) {
    try {
      ...
      if (!user) {
        ...
        if (existingUser) {
          username = `${profile.displayName}-${crypto.randomBytes(3).toString('hex')}`;
        }
        user = new User({
          googleId: profile.id,
          username: username,
          ...
          defaultKey: apiKey,
          isAdmin: false,
          saweriaUserId: "b26c9206-a181-4059-9c24-3afe25473609",
          ipAddress: 0,
        });
        await user.save();
...
app.post('/convert-image', ensureAuthenticated, upload.single('file'), async (req, res) => {
  ...
  try {
    const response = await axios.get(`https://api.kiicodeit.me/ai/toanime?url=${fileUrl}&apikey=KiiCodeAPI2024`);
